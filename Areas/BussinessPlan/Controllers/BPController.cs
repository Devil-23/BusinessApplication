...
namespace BusinessApplication.Areas.BussinessPlan.Controllers
{
    [SessionAuth]
    public class BPController : Controller
    {
        ...
        public ActionResult Income(int year, string jobno)
        {
            ...
            if (check.Count() == 0)
            {
                return RedirectToAction("AccessDenied", "UserAuthorization" ,new { area=""});
            }
            else
            {
                List<string> desiredOrder = new List<string> { "T", "IYR", "IYE", "IYP", "IYM", "IYB", "O" };
                if (existingEntity == null)
                {
                   ...
                   res = res.OrderBy(item => desiredOrder.IndexOf(item.ID)).ToList();
                   model.InList = res;            
                }
                else
                {
        ...
        public ActionResult Income(int year, string jobno, MultipleVM rec)
        {
            ...
            return RedirectToAction("Income", new { year = year, jobno = jobno });
        }
        ...
        [HttpPost]
        public ActionResult MaterialInHouse(int year, string jobno,  List<IncomeVM> InList)
        {
            string User = Session["UserId"].ToString();
            var check = cn.TBL_BP_PERMISSION.Where(x => x.JOB_NO == jobno && x.BP_READ == 1 && x.BP_WRITE == 1 && x.BP_USER == User).ToList();
            if (check.Count() == 0)
        ...
        [HttpPost]
        public ActionResult MaterialBoughtOut(int year, string jobno,  List<IncomeVM> InList)
        {
            string User = Session["UserId"].ToString();
            var check = cn.TBL_BP_PERMISSION.Where(x => x.JOB_NO == jobno && x.BP_READ == 1 && x.BP_WRITE == 1 && x.BP_USER == User).ToList();
            if (check.Count() == 0)
